...
MODEL_PATH = 'model/asl_model.tflite'
if not os.path.exists(MODEL_PATH):
    os.makedirs('model', exist_ok=True)
    gdown.download('https://drive.google.com/uc?id=12f6BXqIhaWN3DTFc32cU92fuk2SqPPJe', MODEL_PATH, quiet=False)
    model_h5 = tf.keras.models.load_model('model/asl_model.h5')
...
def predict_sign(image_bytes):
    ...
    x_off, y_off = (224-new_w)//2, (224-new_h)//2
    ...
    top3 = [{'sign': idx_to_class[i], 'conf': round(float(pred[i])*100, 1)} for i in top3_idx]

    if top3[0]['sign'] in ['blank', 'nothing'] or top3[0]['conf'] < 70:
        return [{'sign': 'nothing', 'conf': 0.0}]
